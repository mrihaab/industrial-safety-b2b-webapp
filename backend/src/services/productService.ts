...
export class ProductService {
  /**
   * Get paginated products with formatted DTOs
   */
  static async getProducts(query: ProductListQuery): Promise<PaginatedProductsResult> {
    ...
    const products: ProductListItemDto[] = await Promise.all(
  ...
  static async getProductBySlug(slug: string): Promise<ProductDetailDto | null> {
    ...
    return {
      ...
      title: product.title,
      slug: product.slug,
      series_name: product.series_name || 'Heavy Duty Series',
      price: Number(product.price),
      moq: product.moq || 50,
      stock_status: product.stock_status || 'IN STOCK',
      status_tag: product.status_tag || 'Safety-System-Active',
      description: product.description,
      rating_score: Number(product.rating_score) || 5.0,

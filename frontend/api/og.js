  ...
  try {
    ...
    if (apiRes.ok) {
      ...
      if (json.success && Array.isArray(json.data) && json.data.length > 0) {
        const featured = json.data.find(p => p.isFeatured || p.is_featured === 1 || p.is_featured === true);
        ...
        if (target) {
          if (target.primaryImage || target.image_url) {
            ...
            // Ensure Cloudinary uses optimal 1200x627 crop transformation if Cloudinary URL
    ...
    console.error("[Vercel OG Prerender Error]:", err);
  }
